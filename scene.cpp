...

{
    ...
    float theta1 = acos(rayDir.normalize() * normN);
    float theta2 = asin((riAir * sin(theta1))/riGlass);

    refractionDir = -cos(theta2) * N;
*/
    float cosi = cos(rayDir * N);
    float etai = 1.000277, etat = 1.458;
    vec3 n = N;
    if (cosi < 0) { cosi = -cosi; }
    else { std::swap(etai, etat); n = -1.0 * N; }
    float eta = etai / etat;
